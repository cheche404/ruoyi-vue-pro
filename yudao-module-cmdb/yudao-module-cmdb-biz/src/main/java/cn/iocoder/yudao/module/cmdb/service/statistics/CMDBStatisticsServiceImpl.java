...
public class CMDBStatisticsServiceImpl implements CMDBStatisticsService {
  ...
  public List<CMDBResourceNumVO> getResourceNum() {
    ...
    for (ResourceType type : ResourceType.values()) {
      ...
      for (CloudType cloudType : CloudType.values()) {
        ...
        Long num = countByCloudArea(mapper, cloudName);  // 你自己写的方法
        innerVO.setNum(num);
  ...
  private Long countByCloudArea(BaseMapperX<?> mapper, String cloudArea) {
    try {
      return mapper.selectCount("cloud_area", cloudArea);
    } catch (Exception e) {
